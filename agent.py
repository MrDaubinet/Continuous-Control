...
OU_SIGMA = 0.05          # Ornstein-Uhlenbeck noise parameter
...
class Agent():
		...
		def act(self, state, add_noise=True):
				...
				state = torch.from_numpy(state).float().to(device)  							# place state on device
				self.actor_local.eval()  																					# evaluatiuon mode
				with torch.no_grad(): 																						# turn off gradients (for speed)
						action = self.actor_local(state).cpu().data.numpy() 					# get the action
				self.actor_local.train()  																				# train the actor
			
				if add_noise:
						action += self.epsilon * self.noise.sample()
				clip = np.clip(action, -1, 1)  																		# clip the Action values
				return clip
